...
const HttpManager = {
    ...
    // 返回指定MV ID的MV
    //TODO: 这里的mv接口还没有写好，先注释掉
    getMvOfId: (id) => get(`mv/detail?id=${id}`),

    // 返回指定歌手名的MV
    mvOfSingerName: (name) => get(`/mv/singer/singername?name=${name}`),

    // 返回指定歌曲名的MV
    mvOfSongName: (name) => get(`/mv/song/songname?name=${name}`),

    //返回指定歌曲id的MV
    getMvOfSongId: (id) => get(`/mv/song/songid?songId=${id}`),

    //返回指定歌手id的MV
    getMvOfSingerId: (id) => get(`/mv/singer/singerid?singerId=${id}`),
